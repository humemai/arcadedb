...
def create_schema(db):
    ...
    try:
        ...
        with db.transaction():
            ...
            db.command("sql", "CREATE PROPERTY Person.ed_date DATE")
            print("  ✓ Created Person properties")
...
def create_sample_data(db):
    ...
    try:
        ...
        people_data = [
            ("Alice Johnson", 28, "New York", "2020-01-15"),
            ("Bob Smith", 32, "San Francisco", "2019-03-20"),
            ("Carol Davis", 26, "Chicago", "2021-06-10"),
            ("David Wilson", 35, "Boston", "2018-11-05"),
            ("Eve Brown", 29, "Seattle", "2020-08-22"),
            ("Frank Miller", 31, "Austin", "2019-12-14"),
            ("Grace Lee", 27, "Denver", "2021-02-28"),
            ("Henry Clark", 33, "Portland", "2019-07-18"),
        ]
        ...
        with db.transaction():
            for name, age, city, joined_date in people_data:
                db.command(
                    "sql",
                ...
                print(f"    ✓ Created person: {name} ({age}, {city})")
        ...
        with db.transaction():
            for person1, person2, since_date, closeness in friendships:
                # Create bidirectional friendship using property-based lookup (like test files)
                db.command(
                    "sql",
                    f"CREATE EDGE FRIEND_OF FROM (SELECT FROM Person WHERE name = '{person1}') "
                ...
                db.command(
                    "sql",
                    f"CREATE EDGE FRIEND_OF FROM (SELECT FROM Person WHERE name = '{person2}') "
...
def demonstrate_sql_queries(db):
    ...
    try:
        ...
        for row in result:
            ...
            print(f"      🏙️  {city}: {count} people, avg age {avg_age:.1f}")
