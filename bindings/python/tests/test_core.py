...
def test_unicode_support(temp_db_path):
    ...
    with arcadedb.create_database(temp_db_path) as db:
        with db.transaction():
            ...
            db.command("sql",
                       "INSERT INTO User SET name = 'José García', "
                       "city = 'São Paulo'")
            db.command("sql",
                       "INSERT INTO User SET name = '王小明', city = '北京'")
            db.command("sql",
                       "INSERT INTO User SET name = '田中太郎', "
                       "city = '東京'")
            db.command("sql",
                       "INSERT INTO User SET name = 'محمد', "
                       "city = 'القاهرة'")
            db.command("sql",
...
def test_large_result_set_handling(temp_db_path):
    ...
    with arcadedb.create_database(temp_db_path) as db:
        ...
        result = db.query(
            "sql",
            "SELECT FROM LargeData WHERE batchNum = 5"
        )
        records = list(result)
...
def test_complex_graph_traversal(temp_db_path):
    ...
    with arcadedb.create_database(temp_db_path) as db:
        ...
        result = db.query(
            "sql",
            "SELECT expand(out('Follows').name) FROM Person "
            "WHERE name = 'Alice'"
        )
        ...
        result = db.query(
            "sql",
            "SELECT expand(in('Follows').name) FROM Person "
            "WHERE name = 'Bob'"
        )
        names = [r.get_property('value') for r in result]
        assert 'Alice' in names
        ...
        result = db.query(
            "sql",
            "SELECT expand(in('Likes').name) FROM Person "
            "WHERE name = 'Bob'"
        )
        names = [r.get_property('value') for r in result]
        assert 'Charlie' in names
